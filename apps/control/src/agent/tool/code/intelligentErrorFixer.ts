...
export function ${pascal}({ className, ...props }: React.ComponentProps<"${tag}">) {
  return React.createElement("${tag}", { className: cn(className), ...props } as React.HTMLAttributes<HTMLElement>);
}
`;
}
...
export function scaffoldMissingModules(
  ...
  for (let rel of missing) {
    ...
    if (fs.existsSync(full) && fs.statSync(full).isDirectory()) {
      const barrel = path.join(full, "index.ts");
    ...
    // resolved file `input.tsx` (then `.ts`).
    const hasExt = /\.(t|j)sx?$/.test(rel);
    if (!hasExt && !fs.existsSync(full)) {
      const tryExt = (["tsx", "ts"] as const).find((e) =>
